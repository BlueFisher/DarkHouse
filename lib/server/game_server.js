...
class gameServer {
    ...
    _onWebSocketConnection(socket) {
        ...
        this._sessionParser(req, {}, () => {
            ...
            if (userId) {
                ...
                if (pair) {
                    log_1.logger.info(`anonymouse user reconnected - ${req.connection.remoteAddress}`);
                    ...
                    log_1.logger.info(`anonymouse user connected - ${req.connection.remoteAddress}`);
                    this._socketPlayerColl.push({
                        sessionId: sessionId,
                        socket: socket
                    });
    ...
    _onInitialize(protocol, socket) {
        ...
        if (pair) {
            ...
            log_1.logger.info(`player [${pair.playerId}]: ${protocol.name} added in game`);
