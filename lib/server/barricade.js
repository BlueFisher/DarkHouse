...
class barricade {
    ...
    adjustCircleCollided(oldPos, newPos, r) {
...
class barricadeManager {
    ...
    didTwoLinesCollided(a, b, c, d) {
        ...
        for (let barricade of this.barricades) {
            ...
            for (let j = 0; j < 4; j++) {
                if (utils.didTwoLinesCross(a, b, vertexes[j], vertexes[(j + 1) % 4])) {
                    for (let k = 0; k < 4; k++) {
                        if (utils.didTwoLinesCross(c, d, vertexes[k], vertexes[(k + 1) % 4])) {
                            isCollided = true;
                            break;
                        }
                    }
    ...
    removeBlockedPlayers(player, playersInSight) {
        for (let i = playersInSight.length - 1; i >= 0; i--) {
            let playerInSight = playersInSight[i];
            let [tangentialPoint1, tangentialPoint2] = utils.getTangentialPointsOfPointToCircle(player.position, playerInSight.position, config.player.radius);
            if (this.didTwoLinesCollided(player.position, tangentialPoint1, player.position, tangentialPoint2)) {
                playersInSight.splice(i, 1);
            }
        }
    }
}
exports.barricadeManager = barricadeManager;
