        ...
        setInterval(() => {
            ...
            for (let i = this._shootingCache.length - 1; i >= 0; i--) {
                ...
                if (this._shootingCache[i].shootedPlayer) {
                    shootedPlayer = this._shootingCache[i].shootedPlayer.getPlayerPROT();
                }
                shootPROTs.push({
            ...
            for (let player of connectedPlayers) {
                ...
                let mainPROT = new toClientPROT.mainPROT(player.getPlayerPROT(), this._getPlayersInPlayerSight(player, config.player.sightRadius).map(p => p.getPlayerPROT()));
