...
class gameCore extends events.EventEmitter {
    ...
    getInitPROT(currPlayerId) {
        ...
        return new toClientPROT.initialize(currPlayerId, players.map(p => p.getPlayerBasicPROT()), this._barricades.map(p => p.getBarricadePROT()), this._propHps.map(p => p.getPropHpPROT()), this._propGuns.map(p => p.getPropGunPROT()));
    }
        ...
        if (firstshootedPlayer) {
            ...
            if (hp - 1 == 0) {
                ...
                if (firstshootedPlayer.getGun().getBullet() > 0) {
                    let newPropGun = new prop_1.propGun(firstshootedPlayer.position, player.getGun());
