...
class gameCore extends events.EventEmitter {
    constructor(width, height, edgeX = 0, edgeY = 0) {
        ...
        this._barricades.push(new barricade_1.barricade(new point(400, 80), new point(420, 400)));
        this._barricades.push(new barricade_1.barricade(new point(80, 400), new point(400, 420)));
    ...
    _playerShooted(player) {
        player.records.shootingInAimTimes++;
        player.records.shootedTimes++;
        let hp = player.getHp();
        if (hp - 1 == 0) {
            player.records.killTimes++;
            this.emit(gameCore.events.gameOver, player.id, new toClientPROT.gameOver(player.records));
