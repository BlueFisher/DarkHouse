...
/**判断点是否在射线的象限范围内 */
function _didDotInRayQuadrant(rayDot, angle, dot) {
 ...
 * 获取射线与圆相交的点
 */
function getRayCircleCollidedPoint(rayDot, angle, circlePoint, radius) {
    let k = Math.tan(angle);
    let b = rayDot.y - k * rayDot.x;
    let d = Math.abs(k * circlePoint.x - circlePoint.y + b) / Math.sqrt(Math.pow(k, 2) + 1);
    if (d > radius)
        return null;
    let footX = (circlePoint.x + k * circlePoint.y - k * b) / (Math.pow(k, 2) + 1);
    let footY = (Math.pow(k, 2) * circlePoint.y + k * circlePoint.x + b) / (Math.pow(k, 2) + 1);
    if (!_didDotInRayQuadrant(rayDot, angle, new point(footX, footY)))
        return null;
    let td = Math.sqrt(Math.pow(radius, 2) - Math.pow(d, 2));
    return new point(footX + Math.cos(angle + Math.PI) * td, footY + Math.sin(angle + Math.PI) * td);
}
exports.getRayCircleCollidedPoint = getRayCircleCollidedPoint;
/**
 * 获取射线与线段相交的点
 */
function getRayLineCollidedPoint(rayDot, angle, vertex1, vertex2) {
...
exports.getRayLineCollidedPoint = getRayLineCollidedPoint;
